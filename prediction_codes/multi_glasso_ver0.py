...
def multi_glasso_train(beta1_current, chrom_betas1, chrom_ld_dict, ld_radius, n_indV, tune_idx, Y, X, lambda1, lambda2, chr_list, num_iter=60):
...
def multi_glasso(data_file_D1, n_indV, init_betas_prefix, ld_radius = None, ld_dict=None, num_iter=60):
    ...
    for fd in range(2):
        ...
        for tune1 in range(l1):
            for tune2 in range(l2):
                ...
                beta1, tune_cor = multi_glasso_train(beta1_current, chrom_betas1, chrom_ld_dict, ld_radius, n_indV, cv_idx[fd], y1, chrom_snps, lambda1[tune1], lambda2[tune2], chr_list) 
                num_non_zeros = 0.0
                for chrom_str in chromosomes_list:
                    if chrom_str in chr_list:
                ...
                if num_non_zeros > 0:
                    ...
                    predicted2 = sp.zeros(cv_ss[(fd+1)%2])
                    for chrom_str in chromosomes_list:
                        if chrom_str in chr_list:
        ...
        beta1_current = init_betas.copy()
        beta1, tune_cor = multi_glasso_train(beta1_current, chrom_betas1, chrom_ld_dict, ld_radius, n_indV, cv_idx[fd], y1, chrom_snps, lambda1[selected[0]], lambda2[selected[1]], chr_list)
        predicted2 = sp.zeros(cv_ss[(fd+1)%2])
        for chrom_str in chromosomes_list:
            if chrom_str in chr_list:
                predicted2 += sp.dot(beta1[chrom_str][:,0], chrom_snps[chrom_str][:,cv_idx[(fd+1)%2]])
...
def main(p_dict):
    ...
    if not os.path.isfile(local_ld_dict_file):
        ...
        f.close()
    now = datetime.now()
    cv_res, cv_lambda, cv_coef = multi_glasso(p_dict["multi_coord"], np.array(p_dict['N']), p_dict['init_betas'], ld_radius = p_dict['ld_radius'], ld_dict=ld_dict, num_iter=60)
    ...
    ff = open("%s_cv_ldradius%d.log"%(p_dict["out"], p_dict['ld_radius']), 'w')
