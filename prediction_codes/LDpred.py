...
Usage: 
python LDpred.py --coord=COORD_DATA_FILE  --ld_radius=LD_RADIUS   --local_ld_file_prefix=LD_FILE_NAME  --PS=FRACTIONS_CAUSAL    
...
#        sys.exit(2)
    ...
    p_dict = {'coord':None, 'ld_radius':None, 'local_ld_file_prefix':None, 'PS':[1,0.3,0.1,0.05,0.03,0.01,0.003,0.001,0.0003,0.0001,0.00001], 'out':None,
...
def ldpred_genomewide(data_file=None, ld_radius = None, ld_dict=None, out_file_prefix=None, ps=None, 
    ...
    print str(float(num_snps))
    print str(n)
    print str(sum_beta2s)
    ...
    out = []
    out_inf = []
    out.append(str(gw_h2_ld_score_est)+'\n')
    ...
    for p in ps:
        ...
        out.append('Starting LDpred with p=%0.4f'%p+'\n')
