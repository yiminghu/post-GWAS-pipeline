#!/usr/bin/env python
"""
Part of codes cited from Bjarni J Vilhjalmsson: https://github.com/bvilhjal
"""
...
#def get_argparser():
#  parser = ArgumentParser(prog="Multi_corrd", 
#                          description="Parse summary stats and reference genotypes for multiple diseases.")
#  parser.add_argument('--gf', required=True,
#                      help="Genotype files")
#  parser.add_argument('--vgf', required=True,
#                      help="Validation genotype files")
#  parser.add_argument('--ssf', required=True,
#                      help="Summary stats")
#  parser.add_argument('--out', required=True,
#                      help="Output file path")
#  parser.add_argument('--vbim', required=True,
#                      help="validation bim files")
#  parser.add_argument('--N', type=True,
#                      help="Sample sizes of different diseases")
#  return parser
...
#             print __doc__
        ...
        for opt, arg in opts:
            ...
            elif opt in ("--N"): p_dict['N'] = [int(item) for item in arg.split(',')]#int(arg)
            else:
...
def parse_sum_stats_multiple(filename=None,
                              bimfile =None,
                              hdf5_file=None, 
                              n=None):
    """
    ...
    hg19chrc    snpid    a1    a2    bp    or1    p1    or2    p2    or3    p3 ....   
    chr1    rs4951859    C    G    729679    0.97853    0.2083    0.97853    0.2083    0.97853    0.2083  
    chr1    rs142557973    T    C    731718    1.01949    0.3298    1.01949    0.3298    1.01949    0.3298  
    ...
            ...
            if chrom.isdigit():
                ...
                if sid in valid_sids:
                    if not chrom in chrom_dict.keys():
                        chrom_dict[chrom] = {'infos':[],'nts': [], 'sids': [], 
                                             'positions': []}
                        chrom_dict[chrom].update({'ps'+str(i):[] for i in range(n_disease)})  
                        chrom_dict[chrom].update({'log_odds'+str(i):[] for i in range(n_disease)})
                        chrom_dict[chrom].update({'betas'+str(i):[] for i in range(n_disease)})            
                    chrom_dict[chrom]['sids'].append(sid)
                    chrom_dict[chrom]['positions'].append(pos)
                    nt = [l[2], l[3]]
                    chrom_dict[chrom]['nts'].append(nt)
                    for di in range(n_disease):
                        raw_beta = sp.log(float(l[5+di*2]))
                        pval = float(l[6+di*2])
                        chrom_dict[chrom]['ps'+str(di)].append(pval)
                        chrom_dict[chrom]['log_odds'+str(di)].append(raw_beta)
                        beta = sp.sign(raw_beta) * stats.norm.ppf(pval/2.0)
                        chrom_dict[chrom]['betas'+str(di)].append(beta / sp.sqrt(n[di]))       
    ...
    for chrom in chrom_dict.keys():
        ...
        to_zip = []
        to_zip.extend([chrom_dict[chrom]['betas'+str(i)] for i in range(n_disease)])
        to_zip.extend([chrom_dict[chrom]['log_odds'+str(i)] for i in range(n_disease)])
        to_zip.extend([chrom_dict[chrom]['ps'+str(i)] for i in range(n_disease)])
        #zip(index, field, *to_zip)
        sl = zip(chrom_dict[chrom]['positions'], chrom_dict[chrom]['sids'], chrom_dict[chrom]['nts'],*to_zip)
        sl.sort()
        ps = []
        betas = []
        nts = []
        ...
        for tlp in sl:
            ...
            betas.append(tlp[3:(3+n_disease)])
            log_odds.append(tlp[(3+n_disease):(3+2*n_disease)])
            ps.append(tlp[(3+2*n_disease):(3+3*n_disease)])
...
#         assert sp.all(g_sids==rg_sids) and sp.all(rg_sids==ss_sids), 'WTF?'
        ...
        maf_adj_prs = sp.dot(log_odds[:,0], raw_snps)
        if has_phenotype:
...
"""
    ...
    if p_dict['ssf_format']=='STANDARD':
        ...
        parse_sum_stats_multiple(filename=p_dict['ssf'], bimfile = bimfile, hdf5_file=h5f, n=p_dict['N'])
